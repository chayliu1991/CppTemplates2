...
只有当`discriminator`匹配时，`VariantChoice::destroy()`操作是有用的。但通常希望不考虑当前活动的类型，直接销毁存储在Variant中的值，因此`Variant::destroy()`将调用基类中所有的`VariantChoice::destroy()`：
...
访问一个没有值的`variant`将抛出EmptyVariant异常，以允许程序从这个异常条件修复。
